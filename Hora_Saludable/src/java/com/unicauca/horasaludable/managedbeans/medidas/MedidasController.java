...
public class MedidasController {
    ...
    public void init() {
        ...
        medicionactual = s.getMedidaactual();
    }
    ...
    public void generarPDF(Usuario user) throws com.lowagie.text.DocumentException {
        try {
            ...
            document.add((Element) new Chunk("Nombre: ", negrilla));
            document.add((Element) new Chunk(user.getUsunombres() + " " + user.getUsuapellidos()));
            document.add((Element) new Chunk("                    Fecha Impresión:" + fecha));

            document.add(new Paragraph(""));
            document.add((Element) new Chunk("Identificacion: ", negrilla));
            document.add((Element) new Chunk(user.getUsuidentificacion() + ""));
    ...
    public void guardarmed() {
        ...
        try {
            ...
            context.addMessage("msggestion", new FacesMessage(FacesMessage.SEVERITY_INFO, "Datos grabados con éxito", "Operacion realizada con exito"));
            ExternalContext extcontext = context.getExternalContext();
            extcontext.getFlash().setKeepMessages(true);
            //extcontext.redirect("GestionTest.xhtml");
        } catch (Exception e) {
    ...
    public void calcularResultadoLeger() {
        ...
        calificacionLeger = calificacionLeger + " (Vo2Max): " + r;
    }
